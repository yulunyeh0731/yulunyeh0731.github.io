...
const MSAL_CONFIG = {
  ...
  cache: {
    ...
    // 整頁跳轉登入時，MSAL 出發前會把核對值（state）存起來，跳回來要用它比對。
    // 只存在 sessionStorage 的話，某些瀏覽器在跳轉過程中會把它丟掉，
    // 回來就變成 state_not_found，而且會留下殘留讓之後每次按登入都沒反應。
    // 額外存一份到 cookie 可以撐過跳轉。權杖本身仍然只在 sessionStorage，
    // 關掉分頁就消失，不會留在這台電腦上。
...
async function signIn() {
  ...
  try {
    ...
    // interaction_in_progress 表示上次的跳轉沒走完，MSAL 自己把這次攔下來了。
    // 這種情況畫面上會完全沒反應，使用者只會以為按鈕壞了。
    // 清掉殘留再試一次，不要求使用者去開開發者工具。
    if (code.indexOf('interaction_in_progress') > -1 || code.indexOf('state_not_found') > -1) {
      if (hint) hint.textContent = '正在重設登入狀態…';
      clearMsalState();
      try {
        await msalApp.loginRedirect({ scopes: SCOPE_RO });
        return;
      } catch (e2) {
        if (hint) hint.innerHTML = '登入失敗：' + (e2.message || e2)
          + '<br>請把這個網站的分頁全部關掉再重新開啟。';
        return;
      }
    }
...
// 清掉 MSAL 的殘留狀態。只動 MSAL 自己的鍵，不碰別的東西。
// 用途：上次跳轉登入沒走完（state_not_found、interaction_in_progress）時，
// 那些殘留會讓後續每次按登入都被 MSAL 自己攔下來，畫面上完全沒反應。
function clearMsalState() {
  const isMsalKey = k => k.indexOf('msal') === 0 || k.indexOf('server-telemetry') === 0
                      || k.indexOf(MSAL_CONFIG.auth.clientId) > -1;
  for (const store of [sessionStorage, localStorage]) {
    try {
      Object.keys(store).filter(isMsalKey).forEach(k => store.removeItem(k));
    } catch (e) { /* 隱私設定可能擋住存取，擋住就算了 */ }
  }
  // 核對值的 cookie 也要清，否則下一次會拿到過期的值
  try {
